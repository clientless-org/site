export const prerender = true;

const favicon = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 128 128" role="img" aria-label="Clientless">
  <rect width="128" height="128" rx="28" fill="#101820"/>
  <path d="M82 31h16v66H82z" fill="#32d296"/>
  <path d="M83 47c-6-10-16-16-29-16-21 0-36 15-36 37s15 37 36 37c13 0 23-6 29-16" fill="none" stroke="#f6f1e8" stroke-width="15" stroke-linecap="round" stroke-linejoin="round"/>
</svg>`;

export function GET() {
  return new Response(favicon, {
    headers: {
      "Content-Type": "image/svg+xml; charset=utf-8",
    },
  });
}
